...
module.exports = {
  ...
  getRole: ({_context})=> {
    let {getRole} = require('./services.js')
    getRole({_context}).then((data) => {
    _context.res.json(data)
    })
  },
  getUser: ({_context})=> {
    let {getUser} = require('./services.js')
    getUser({_context}).then((data) => {
    _context.res.json(data)
    })
  },
  getModule: ({_context})=> {
    let {getModule} = require('./services.js')
    getModule({_context}).then((data) => {
    _context.res.json(data)
    })
  }
}