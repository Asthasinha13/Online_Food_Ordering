...
export default {
  ...
  data () {
    return {
      columns: [],
      visibleColumns: ['action_name', 'module_name', 'status', 'btnAction'],
      data: []
    }
  },
  computed: {
  ...
  methods: {
    ...
    deleteItem (data) {
      this.$q
        ...
        .onOk(() => {
          this.$api.delete("/api/actions?dataId=" + data.row.id)
            .then((response) => {
    ...
    getData () {
      this.$api.get("/api/actions/").then((response) => {
        console.log(response.data);
        this.data = response.data;
      });
    },
    getColumns () {
      this.$api.get("/api/actions/getColumns").then((response) => {
