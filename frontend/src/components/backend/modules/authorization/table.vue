...
export default {
  ...
  data () {
    return {
      columns: [],
      visibleColumns: ['role_name', 'action_name', 'module_name', 'permission', 'btnAction'],
      data: []
    }
  },
  computed: {
  ...
  methods: {
    ...
    deleteItem (data) {
      this.$q
        ...
        .onOk(() => {
          this.$api.delete("/api/authorizations?dataId=" + data.row.id)
            .then((response) => {
    ...
    getData () {
      this.$api.get("/api/authorizations/").then((response) => {
        console.log(response.data);
        this.data = response.data;
      });
    },
    getColumns () {
      this.$api.get("/api/authorizations/getColumns").then((response) => {
