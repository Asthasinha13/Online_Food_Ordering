...
export default {
  ...
  data () {
    return {
      columns: [],
      visibleColumns: ['staff_name', 'role', 'contact_no', 'email', 'btnAction'],
      data: []
    }
  },
  computed: {
  ...
  methods: {
    ...
    deleteItem (data) {
      this.$q
        ...
        .onOk(() => {
          this.$api.delete("/api/delivery-staffs?dataId=" + data.row.id)
            .then((response) => {
    ...
    getData () {
      this.$api.get("/api/delivery-staffs/").then((response) => {
        console.log(response.data);
        this.data = response.data;
      });
    },
    getColumns () {
      this.$api.get("/api/delivery-staffs/getColumns").then((response) => {
