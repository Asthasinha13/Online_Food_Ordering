<template>
  <div class="q-pa-md bg-orange-1 fit">
    <div class="q-gutter-md items-start justify-start">
      ...
      <table-module @open-view="({ context }) => { view.context = context; view.open = true }"
        @edit="({ context }) => { form.data = context; form.mode = 'Edit'; form.open = true }"></table-module>
    </div>
    <q-dialog v-model="form.open">
      <form-module :formData="form.data" :mode="form.mode" @update-complete="
  form.open = false;
...
export default {
  name: "moduleModule",
  components: { formModule, tableModule },
  data () {
    return {
      form: {
        open: false,
