...
export default {
  ...
  data () {
    return {
      columns: [],
      visibleColumns: ['module_name', 'status', 'btnAction'],
      data: []
    }
  },
  computed: {
  ...
  methods: {
    ...
    deleteItem (data) {
      this.$q
        ...
        .onOk(() => {
          this.$api.delete("/api/modules?dataId=" + data.row.id)
            .then((response) => {
    ...
    getData () {
      this.$api.get("/api/modules/").then((response) => {
        console.log(response.data);
        this.data = response.data;
      });
    },
    getColumns () {
      this.$api.get("/api/modules/getColumns").then((response) => {
