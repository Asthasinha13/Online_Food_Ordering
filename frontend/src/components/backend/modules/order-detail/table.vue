...
export default {
  ...
  data () {
    return {
      columns: [],
      visibleColumns: ['restaurant_name', 'contact_number', 'email', 'btnAction'],
      data: []
    }
  },
  computed: {
  ...
  methods: {
    ...
    deleteItem (data) {
      this.$q
        ...
        .onOk(() => {
          this.$api.delete("/api/order-details?dataId=" + data.row.id)
            .then((response) => {
    ...
    getData () {
      this.$api.get("/api/order-details/").then((response) => {
        console.log(response.data);
        this.data = response.data;
      });
    },
    getColumns () {
      this.$api.get("/api/order-details/getColumns").then((response) => {
