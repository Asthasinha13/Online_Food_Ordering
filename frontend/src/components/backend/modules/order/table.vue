...
export default {
  ...
  data () {
    return {
      columns: [],
      visibleColumns: ['order_date', 'customer_name', 'contact_no', 'btnAction'],
      data: []
    }
  },
  computed: {
  ...
  methods: {
    ...
    deleteItem (data) {
      this.$q
        ...
        .onOk(() => {
          this.$api.delete("/api/orders?dataId=" + data.row.id)
            .then((response) => {
    ...
    getData () {
      this.$api.get("/api/orders/").then((response) => {
        console.log(response.data);
        this.data = response.data;
      });
    },
    getColumns () {
      this.$api.get("/api/orders/getColumns").then((response) => {
