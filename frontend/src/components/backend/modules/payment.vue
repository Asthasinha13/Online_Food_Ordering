  ...
  <div class="q-pa-md bg-orange-1 fit">
    <div class="q-gutter-md items-start justify-start">
      <div class="row items-center q-gutter-md">
        <div>
          <h3 class="q-ma-none">Payments</h3>
        </div>
        <div>
      ...
      <table-payment @open-view="({ context }) => { view.context = context; view.open = true }"
        @edit="({ context }) => { form.data = context; form.mode = 'Edit'; form.open = true }"></table-payment>
    </div>
    <q-dialog v-model="form.open">
      <form-payment :formData="form.data" :mode="form.mode" @update-complete="
  form.open = false;
...
import tablePayment from "./payment/table";
export default {
  name: "modulePayment",
  components: { formPayment, tablePayment },
  data () {
    return {
      form: {
        open: false,
