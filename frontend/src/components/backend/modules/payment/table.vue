...
export default {
  ...
  data () {
    return {
      columns: [],
      visibleColumns: ['order_date', 'amount', 'customer_name', 'contact_no', 'btnAction'],
      data: []
    }
  },
  computed: {
  ...
  methods: {
    ...
    deleteItem (data) {
      this.$q
        ...
        .onOk(() => {
          this.$api.delete("/api/payments?dataId=" + data.row.id)
            .then((response) => {
    ...
    getData () {
      this.$api.get("/api/payments/").then((response) => {
        console.log(response.data);
        this.data = response.data;
      });
    },
    getColumns () {
      this.$api.get("/api/payments/getColumns").then((response) => {
