...
export default {
  ...
  data () {
    return {
      columns: [],
      visibleColumns: ['name', 'username', 'email', 'btnAction'],
      data: []
    }
  },
  computed: {
  ...
  methods: {
    ...
    deleteItem (data) {
      this.$q
        ...
        .onOk(() => {
          this.$api.delete("/api/users?dataId=" + data.row.id)
            .then((response) => {
    ...
    getData () {
      this.$api.get("/api/users/").then((response) => {
        console.log(response.data);
        this.data = response.data;
      });
    },
    getColumns () {
      this.$api.get("/api/users/getColumns").then((response) => {
